...
const outfitController = {
    ...
    deleteOutfit: async (req, res) => {
        try {
            ...
            console.error(err)
            res
                .status(err.message === "Outfit not found or unauthorized" ? 404 : 500)
                .json({ message: "Error deleting outfit" })
        }
    },
    ...
    addComment: async (req, res) => {
        try {
            const outfit = await outfitService.addComment(req.params.id, req.userId, req.body.text)
            res.json(outfit)
        } catch (err) {
